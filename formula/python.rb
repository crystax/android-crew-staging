class Python < Library
  ...
  def build_for_abi(abi, toolchain, release, dep_dirs)
    ...
    config_site = "#{config_dir}/config.site"
    gen_config_site config_site, major_ver

    # Step 1: configure
    #
    python_for_build = "#{Global::NDK_DIR}/prebuilt/#{File.basename(Global::TOOLS_DIR)}/bin/python"
    ...
    FileUtils.cd(config_dir) { system "#{src_dir}/configure", *args }
    #build_env.clear

    # Step 2: build python-core
    # todo: use some dir inside crew repository for python support dir
    support_dir = "#{Global::NDK_DIR}/build/tools/build-target-python"
