...
class TestInstallOptions < MiniTest::Test
  ...
  def test_initialize
    # empty ctor
    h = { platform: Global::PLATFORM_NAME, check_shasum: true, cache_only: false, ignore_cache: false, force: false, all_versions: false, with_dev_files: false }
    ...
    assert_equal(false,                 v.ignore_cache?)
    ...
    # all options
    h = { platform: 'darwin-x86_64', check_shasum: false, cache_only: true, ignore_cache: false, force: true, all_versions: true, with_dev_files: true }
    ...
    # conflicting options
    assert_raises(IncompatibleOptions) { Crew::Install::Options.new(['--cache-only',   '--ignore-cache']) }
    assert_raises(IncompatibleOptions) { Crew::Install::Options.new(['--ignore-cache', '--cache-only'])   }
